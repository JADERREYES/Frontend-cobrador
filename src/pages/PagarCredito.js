...
export default function PagarCredito() {
  ...
  const cargarPrestamo = async () => {
    try {
      ...
      const cuota = prestamoData.totalAPagar / (prestamoData.plazo || prestamoData.numeroCuotas || 30);
      ...
      const esperado = cuotaRedondeada * Math.min(diffDays, prestamoData.plazo || prestamoData.numeroCuotas || 30);
  ...
  const handlePagar = async (e) => {
    ...
    try {
      ...
      const pagoData = {
        ...
        monto: montoNum,
        metodoPago: 'efectivo',
        referencia: `Pago de ${fmt(montoNum)}`
      };
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#f1f5f9", padding: "20px" }}>
        <div style={{ maxWidth: "500px", margin: "0 auto", background: "#fff", borderRadius: "16px", padding: "24px", textAlign: "center" }}>
          ...
          <button onClick={() => navigate(-1)} style={{ padding: "12px 24px", background: "#0d9488", color: "#fff", border: "none", borderRadius: "10px", cursor: "pointer" }}>
            Volver
          </button>
        </div>
      </div>
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#f1f5f9", padding: "20px" }}>
        <div style={{ maxWidth: "500px", margin: "0 auto", background: "#fff", borderRadius: "16px", padding: "24px", textAlign: "center" }}>
          ...
          <button onClick={() => navigate(-1)} style={{ marginTop: "20px", padding: "12px 24px", background: "#0d9488", color: "#fff", border: "none", borderRadius: "10px", cursor: "pointer" }}>
            Volver
          </button>
        </div>
      </div>
  ...
  const cliente = prestamo.clienteId || {};
  const totalAPagar = prestamo.totalAPagar;
  const totalPagado = prestamo.totalPagado || 0;
  const saldoPendiente = totalAPagar - totalPagado;
      ...
      <div style={{ maxWidth: "500px", margin: "0 auto" }}>
        ...
        <div style={{ background: "#fff", borderRadius: "16px", padding: "20px", marginBottom: "16px", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "12px" }}>
            ...
            <span style={{ fontWeight: "600", color: "#1e293b" }}>{prestamo.plazo || prestamo.numeroCuotas || 30} días</span>
          </div>
          
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "12px" }}>
            <span style={{ color: "#64748b" }}>Cuota diaria:</span>
            <span style={{ fontWeight: "600", color: "#0d9488", fontSize: "16px" }}>{fmt(cuotaDiaria)}</span>
          </div>
          ...
          <div style={{ height: "8px", background: "#f1f5f9", borderRadius: "4px", marginTop: "8px", overflow: "hidden" }}>
            <div style={{ height: "100%", width: `${porcentajePagado}%`, background: porcentajePagado >= 100 ? "#22c55e" : "#0d9488", borderRadius: "4px" }} />
          </div>
          ...
          <form onSubmit={handlePagar}>
            ...
            <div style={{ display: "flex", gap: "12px", marginTop: "24px" }}>
              ...
              <button
                ...
                style={{ flex: 1, padding: "14px", background: "linear-gradient(135deg, #0d9488, #0f766e)", border: "none", borderRadius: "10px", fontSize: "16px", fontWeight: "600", color: "#fff", opacity: (saving || saldoPendiente <= 0) ? 0.6 : 1, cursor: "pointer" }}
              >
